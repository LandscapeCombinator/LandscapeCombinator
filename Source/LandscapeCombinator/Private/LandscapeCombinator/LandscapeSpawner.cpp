...
bool ALandscapeSpawner::SpawnLandscape(FName SpawnedActorsPathOverride, bool bIsUserInitiated, TObjectPtr<ALandscape>& OutLandscape, TArray<ADecalActor*>& OutDecals)
{
	...
	HMFetcher* Fetcher = HeightmapDownloader->CreateFetcher(
		...
		true,
		true,
		SpawnMethod == ESpawnMethod::CreateFreshLandscape || SpawnMethod == ESpawnMethod::CreateFreshLandscapeIncrementally, // convert to PNG only for non-extension
	...
	if (SpawnMethod == ESpawnMethod::CreateFreshLandscape || SpawnMethod == ESpawnMethod::CreateFreshLandscapeIncrementally)
	{
		if (!Concurrency::RunOnGameThreadAndWait([&]() {
			...
			if (SpawnMethod == ESpawnMethod::CreateFreshLandscapeIncrementally)
			{
				...
				FString NewFile = SubFolder / FPaths::GetCleanFilename(Files[0]);
			...
			UE_LOG(LogTemp, Error, TEXT("Coordinates:"));
			UE_LOG(LogTemp, Error, TEXT("MinX: %f"), (*Coordinates)[0]);
			UE_LOG(LogTemp, Error, TEXT("MaxX: %f"), (*Coordinates)[1]);
			UE_LOG(LogTemp, Error, TEXT("MinY: %f"), (*Coordinates)[2]);
			UE_LOG(LogTemp, Error, TEXT("MaxY: %f"), (*Coordinates)[3]);
	...
	if (SpawnMethod == ESpawnMethod::CreateFreshLandscapeIncrementally)
	{
		// extend the SpawnedLandscape with all files except the first
		if (!Concurrency::RunOnGameThreadAndWait([&]() {
			TArray<FString> Files2 = Files;
			Files2.RemoveAt(0);
			return LandscapeUtils::ExtendLandscape(OutLandscape, Files2, true);
		}))
			return false;
	}
	else if (SpawnMethod == ESpawnMethod::ExtendExistingLandscape)
	{
		// extend LandscapeToExtent with all files
		if (!Concurrency::RunOnGameThreadAndWait([&]() {
			return LandscapeUtils::ExtendLandscape(LandscapeToExtend, Files, bUpdateAllOverlappingComponents);
		}))
			return false;
	}
