...
{
	if (IsValid(SpawnedLandscape))
	{
		TArray<ALandscapeStreamingProxy*> LandscapeStreamingProxies = LandscapeUtils::GetLandscapeStreamingProxies(SpawnedLandscape);
				...
				if (IsValid(SpawnedLandscape))	
				{
					...
					switch (DecalCreation)
					{
						case EDecalCreation::None:
						{
							...
							if (OnComplete) OnComplete(SpawnedLandscape);
							return;
						}
						...
						case EDecalCreation::MapTiler:
						{
							...
							DecalDownloader->ParametersBoundingActor = SpawnedLandscape;
