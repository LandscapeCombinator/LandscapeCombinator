...
void HMLitto3DGuadeloupe::Fetch(int InputEPSG, TArray<FString> InputFiles, TFunction<void(bool)> OnComplete)
{
	...
	FScopedSlowTask ExecProcessTask(FileNamesNum, LOCTEXT("ExtractTask", "Landscape Combinator: Extracting Litto3D Files"));
	...
	for (auto& FileName : FileNames)
	{
		...
		FString ExtractionDir = FPaths::Combine(Directories::LandscapeCombinatorDir(), FPaths::GetBaseFilename(FileName));
		...
		{
			UE_LOG(LogLandscapeCombinator, Error, TEXT("Could not extract %s."), *ArchiveFile);
			if (OnComplete) OnComplete(false);
			return;
		}
